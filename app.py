...
from database.utility import checkUserStatus, getPassowordFromDB,updatePassword, deleteNoteFromDB
from database.utility import addNotesInDB, getNotesFromDB, getNoteById, updateNoteInDB
...
# to get emial from login-auth token
...
def dashboard(token):
    ...
    return render_template('dashboard.html', token=token, notes = notes)

# add new note
@app.route('/dashboard/addnote/<token>', methods=['GET','POST'])
def add_notes(token):
    email = verify_login_token(token)
    if not email:
        return redirect(url_for('login'))
...
def edit_note(note_id, token):
    ...
    if request.method == 'POST':
        ...
        if updateNoteInDB(note_id, email, title, content):
            return redirect(url_for('dashboard', token=token))
 
    note = getNoteById(note_id, email)
